...
echo "1786" | sudo -S rm -f ${CHROOT_PATH:-/home/john/zforge_workspace/chroot}/var/lib/dpkg/lock*
echo "1786" | sudo -S rm -f ${CHROOT_PATH:-/home/john/zforge_workspace/chroot}/var/cache/apt/archives/lock

# Create the apt-key wrapper directly
echo "1786" | sudo -S tee ${CHROOT_PATH:-/home/john/zforge_workspace/chroot}/usr/bin/apt-key > /dev/null << 'EOF'
#!/bin/bash
...
echo "1786" | sudo -S chmod +x ${CHROOT_PATH:-/home/john/zforge_workspace/chroot}/usr/bin/apt-key

# Create symlink
echo "1786" | sudo -S ln -sf /usr/bin/apt-key ${CHROOT_PATH:-/home/john/zforge_workspace/chroot}/usr/local/bin/apt-key 2>/dev/null || true

# Test DNS resolution in chroot
echo "Testing chroot DNS resolution..."
echo "1786" | sudo -S chroot ${CHROOT_PATH:-/home/john/zforge_workspace/chroot} /bin/bash -c 'ping -c 1 deb.debian.org' && echo "✓ DNS working" || echo "✗ DNS still not working"
