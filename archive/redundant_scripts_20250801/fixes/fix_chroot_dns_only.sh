...
create_hosts_workaround() {
    ...
    if [ -n "$primary_ip" ]; then
        ...
        sudo tee -a ${CHROOT_PATH:-/home/john/zforge_workspace/chroot}/etc/hosts > /dev/null << EOF
...
configure_apt_for_hosts() {
    ...
    sudo tee ${CHROOT_PATH:-/home/john/zforge_workspace/chroot}/etc/apt/apt.conf.d/99-zforge-hosts > /dev/null << 'EOF'
...
main() {
    ...
    if ! mountpoint -q ${CHROOT_PATH:-/home/john/zforge_workspace/chroot}/proc 2>/dev/null; then
        echo "   Mounting chroot filesystems..."
        sudo mount -t proc proc ${CHROOT_PATH:-/home/john/zforge_workspace/chroot}/proc
        sudo mount -t sysfs sysfs ${CHROOT_PATH:-/home/john/zforge_workspace/chroot}/sys
        sudo mount --bind /dev ${CHROOT_PATH:-/home/john/zforge_workspace/chroot}/dev
        sudo mount -t devpts devpts ${CHROOT_PATH:-/home/john/zforge_workspace/chroot}/dev/pts
    fi
