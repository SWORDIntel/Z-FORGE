...
mount_chroot() {
    ...
    local mounts=(
        "proc:proc:/tmp/zforge_workspace/chroot/proc"
        "sysfs:sys:/tmp/zforge_workspace/chroot/sys"
        "bind:dev:/tmp/zforge_workspace/chroot/dev"
        "devpts:dev/pts:/tmp/zforge_workspace/chroot/dev/pts"
    )
...
configure_apt() {
    ...
    sudo tee /tmp/zforge_workspace/chroot/etc/apt/apt.conf.d/99-zforge-network > /dev/null << 'EOF'
...
test_dns() {
    ...
    if sudo chroot /tmp/zforge_workspace/chroot /bin/bash -c 'nslookup deb.debian.org >/dev/null 2>&1'; then
...
main() {
    ...
    echo "Target: Z-FORGE chroot at /tmp/zforge_workspace/chroot"
    echo
    
    # Check if chroot exists
    if [ ! -d "/tmp/zforge_workspace/chroot" ]; then
        echo "✗ Chroot directory not found at /tmp/zforge_workspace/chroot"
        exit 1
    fi
