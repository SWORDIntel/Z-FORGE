...
MOUNTS=$(mount | grep "/tmp/zforge_workspace" | awk '{print $3}' | sort -r)
...
if [ -n "$MOUNTS" ]; then
    ...
    echo "sudo umount -l /tmp/zforge_workspace/*/proc"
    echo "sudo umount -l /tmp/zforge_workspace/*/sys" 
    echo "sudo umount -l /tmp/zforge_workspace/*/dev/pts"
    echo "sudo umount -l /tmp/zforge_workspace/*/dev"
else
...
echo "sudo mv /tmp/zforge_workspace \$HOME/"
...
cat << 'EOF'
# Unmount all
sudo umount -l /tmp/zforge_workspace/*/proc || true
sudo umount -l /tmp/zforge_workspace/*/sys || true
sudo umount -l /tmp/zforge_workspace/*/dev/pts || true
sudo umount -l /tmp/zforge_workspace/*/dev || true
